...
function buildReviewPrompt(input: {
  ...
  return renderPromptVariant({
    ...
    extraInstructions: [
      'Review the implementation based on the evidence provided below. Return JSON only.',
      ...
      'IMPORTANT: You are reviewing evidence, not re-running checks. The deterministic checks (cargo test, clippy, fmt, forge test) have ALREADY been run by a separate evaluator. Their results are in the evidence below. Trust those results. Do NOT attempt to run commands yourself.',
      'REVIEW CHECKLIST (from evidence only):',
      '- Do the deterministic check results in the evidence show all passing? If yes, checks are verified.',
      '- From the track results and stdout evidence: does the implementation address the full scope?',
      '- Are there any obvious code quality issues visible in the evidence (duplicated logic, missing error handling, placeholder code)?',
      '- Does the implementation summary align with the success criteria?',
      'SCORING: quality and correctness are 0-10 based on the evidence provided.',
      'If deterministic checks all pass AND the implementation covers the requested scope, recommend complete.',
      'If deterministic checks fail, recommend repair with the specific failing check.',
      'Only recommend escalate for genuine strategic ambiguity, never because you cannot run commands.',
    ],
  });
}
