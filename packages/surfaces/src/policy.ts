...
function findProjectContext(projectPath: string): string | null {
  // Look for ChatGPT thread or context files in parent directories
  const candidates = [
  ...
  for (const dir of candidates) {
    try {
      ...
      // Find .txt files (ChatGPT exports) or master .md files
...
async function executeSpawnSession(action: Action): Promise<ActionOutcome> {
  ...
  try {
    ...
    nodeFs.mkdirSync(join(FOREMAN_HOME, 'logs'), { recursive: true })

    // 1. Generate project-specific CLAUDE.md from ChatGPT thread if it doesn't exist
    const claudeMdPath = join(projectPath, 'CLAUDE.md')
    if (!existsSync(claudeMdPath)) {
      const contextFile = findProjectContext(projectPath)
      if (contextFile) {
        const contextSummary = nodeFs.readFileSync(contextFile, 'utf8').slice(0, 3000)
        nodeFs.writeFileSync(claudeMdPath, `# Project Context (auto-generated by Foreman)

## Source
This project context was extracted from the original design document.
...
`, 'utf8')
    ...
    // 3. Capture session output to log file
    ...
    // 4. Write the driver script with round-aware continuation
    const driverFile = join(FOREMAN_HOME, 'tmp', `${sessionName}-driver.sh`)
    const progressLog = join(FOREMAN_HOME, 'logs', `progress-${sessionName}.log`)
    const sessionState = join(projectPath, '.foreman', 'session-state.md')
...
echo "[foreman] Starting work on $PROJECT at $(date -Iseconds)" | tee -a "$PROGRESS_LOG"

for ROUND in $(seq 1 20); do
  echo ""
  echo "[foreman] === Round $ROUND / 20 ===" | tee -a "$PROGRESS_LOG"

  # Read previous round state if it exists
  CONTEXT=""
  ...
  PROMPT="\${CONTEXT}You are working on this project autonomously. Round $ROUND of 20.

If CLAUDE.md exists in this repo, READ IT FIRST for project context.
...
WRITE CODE. COMMIT IT. Do not just analyze."
  ...
  $CLAUDE -p "$PROMPT" --output-format text --dangerously-skip-permissions 2>&1 || true

  # Log progress
  COMMITS=$(git rev-list --count HEAD 2>/dev/null || echo 0)
  CHANGED=$(git diff --stat HEAD 2>/dev/null | tail -1)
  echo "[foreman] Round $ROUND complete. Total commits: $COMMITS. Working tree: \${CHANGED:-clean}" | tee -a "$PROGRESS_LOG"

  # If session-state says DONE, stop
  if [ -f "$SESSION_STATE" ] && grep -qi "DONE\\|COMPLETE\\|PRODUCTION.READY" "$SESSION_STATE" 2>/dev/null; then
    echo "[foreman] Project marked as done in session-state.md" | tee -a "$PROGRESS_LOG"
    break
  fi
...
echo "[foreman] Session complete at $(date -Iseconds)" | tee -a "$PROGRESS_LOG"
`, 'utf8')
