...
Quality bar: Senior staff engineer (L7/L8). Zero tolerance for slop, overengineering, or bloat.

- **Succinctness**: Fewer lines is better. Minimal code. Match existing codebase style exactly.
- **No mocks**: Favor real integration tests over mocked tests. If you must mock, mock only external services, never internal logic.
- **No placeholders**: Complete every implementation fully. No TODOs, no stubs, no "left as exercise."
- **No hand-holding**: Never ask "should I?" or "want me to?" — just do it. Make the right call.
- **Complete tasks fully**: Run tests. Verify everything works before declaring done.
- **Self-assess**: After completing work, rate quality 1-10. If below 9, keep improving.
- **TypeScript**: strict mode, no semicolons, single quotes, 2-space indent.
- **Python**: type hints, docstrings, pytest over unittest.
- **Commits**: Conventional commits (feat:, fix:, chore:). Descriptive messages. Commit frequently.
- **Tests**: Write tests for everything. Run them. Fix failures before moving on. Integration > unit > mock.
- **Never dismiss failures**: Never say "pre-existing" or "not from my changes." Fix everything you find.
